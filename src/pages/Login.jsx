import { useEffect, useState, useRef } from 'react';
import { useNavigate, useLocation } from 'react-router-dom';
import { useAuth } from '../contexts/AuthContext.jsx';
import { useTokenClient } from '../hooks/useAuth.js';

export default function Login() {
  const { auth, loading } = useAuth();
  const { requestToken } = useTokenClient();
  const navigate = useNavigate();
  const location = useLocation();
  const [error, setError] = useState('');
  const [signingIn, setSigningIn] = useState(false);
  const from = location.state?.from || '/';

  useEffect(() => {
    if (!loading && auth && !auth.denied && !auth.error) {
      const dest = redirectForRoles(auth.roles, auth.bootstrap);
      navigate(dest, { replace: true });
    }
    if (!loading && auth?.denied) {
      navigate('/access-denied', { replace: true, state: { email: auth.email } });
    }
    if (!loading && auth?.error) {
      setError(`Error del sistema: ${auth.error}`);
      setSigningIn(false);
    }
  }, [auth, loading, navigate]);

  function redirectForRoles(roles = [], bootstrap = false) {
    if (bootstrap && roles.includes('ADMIN')) return '/setup';
    if (from && from !== '/login' && from !== '/') return from;
    if (roles.includes('TUTOR')) return '/tutor/dashboard';
    if (roles.includes('COORD')) return '/coord/panel';
    if (roles.includes('ASISTENTE')) return '/coord/panel';
    return '/tutor/dashboard';
  }

  function handleLogin() {
    setError('');
    setSigningIn(true);
    requestToken(
      () => setSigningIn(false),
      (err) => { setError(err.message || 'Error al iniciar sesión'); setSigningIn(false); }
    );
  }

  if (loading) return (
    <div className="min-h-screen flex items-center justify-center" style={{ background: 'var(--color-fondo)' }}>
      <div className="w-8 h-8 border-2 border-[var(--color-verde)] border-t-transparent rounded-full animate-spin" />
    </div>
  );

  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4" style={{ background: 'var(--color-fondo)' }}>
      <div className="w-full max-w-md">
        {/* Card */}
        <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
          {/* Header band */}
          <div className="h-2" style={{ background: 'var(--color-verde)' }} />
          <div className="px-8 py-8 flex flex-col items-center gap-6">
            {/* Logo */}
            <div className="flex flex-col items-center gap-2">
              <div className="w-16 h-16 rounded-2xl flex items-center justify-center text-white text-2xl font-bold shadow-lg"
                style={{ background: 'var(--color-verde)' }}>
                SST
              </div>
              <h1 className="text-xl font-bold text-gray-800 text-center">Sistema de Seguimiento Tutorial</h1>
              <p className="text-sm text-gray-500 text-center">CPEIP · Mejorando la Gestión Escolar con IA</p>
            </div>

            {/* Divider */}
            <div className="w-full border-t border-gray-100" />

            {/* Login */}
            <div className="flex flex-col items-center gap-4 w-full">
              <p className="text-sm text-gray-600 text-center">
                Inicia sesión con tu cuenta Gmail registrada en el sistema.
              </p>

              <button
                onClick={handleLogin}
                disabled={signingIn}
                className="flex items-center gap-3 px-6 py-3 rounded-xl border-2 border-gray-200 bg-white hover:border-[var(--color-verde)] hover:shadow-md transition-all font-medium text-gray-700 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {signingIn ? (
                  <div className="w-5 h-5 border-2 border-[var(--color-verde)] border-t-transparent rounded-full animate-spin" />
                ) : (
                  <GoogleIcon />
                )}
                {signingIn ? 'Conectando…' : 'Continuar con Google'}
              </button>

              {error && (
                <div className="w-full bg-red-50 border border-red-200 rounded-lg px-4 py-3 text-sm text-red-700">
                  {error}
                </div>
              )}
            </div>
          </div>
        </div>

        <p className="text-center text-xs text-gray-400 mt-6">
          Solo usuarios registrados pueden acceder · {new Date().getFullYear()}
        </p>
      </div>
    </div>
  );
}

function GoogleIcon() {
  return (
    <svg width="20" height="20" viewBox="0 0 24 24">
      <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
      <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
      <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
      <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
    </svg>
  );
}
